...
def csc_database_etl():
    ...
    def merge_csc(map_app: MapCsc):
        ...
        return {'table_name': map_app.CSC_MERGE_TABLE, 'df_value': map_app.merge_multi_data()}  # 传递到下一个子任务
    ...
    def start_tasks(csc_merge_table):
        ...
        # logging.error(MAP.MULTI_MAP_TABLES)
        # i = MAP.MULTI_MAP_TABLES[0]
        # load_feather(transform_df(extract_sql(i[0], i[1], i[2], i[3], 20210101, 20230101)))
        ...
        # 2.生成合并表
        res = load_feather(transform_df(merge_csc(MAP)))
        #  3.检查数据准确性
        check_merge(res)
